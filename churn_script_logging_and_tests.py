...
def test_encoder_helper():
    ...
    try:
        df_raw = import_data("./data/bank_data.csv")
        enc_cols_size = sum([len(np.unique(df_raw[cat]))
                             for cat in category_lst]) - len_cols
        df_encoded = encoder_helper(
            import_data("./data/bank_data.csv"), category_lst)
        assert df_encoded.shape[1] == enc_cols_size
        logging.info("Testing encoder_helper: SUCCESS")
    except AssertionError as err:
        logging.info(
            "Testing encoder_helper: The dataframe does not appear to have the correct number of encoded categorical columns")
        raise err
...
def test_perform_feature_engineering():
    ...
    try:
        df_raw = import_data("./data/bank_data.csv")
        (_, x_test, _, _) = perform_feature_engineering(df_raw)
        logging.info(
            'Testing perform_feature_engineering. Data split successful: SUCCESS')
    except Exception as err:
        logging.error(
            'Testing perform_feature_engineering failed. Error msg: %s',
            str(err))
...
def test_train_models():
    ...
    try:
        df_raw = import_data("./data/bank_data.csv")
        (x_train, x_test, y_train, y_test) = perform_feature_engineering(df_raw)
        train_models(x_train, x_test, y_train, y_test)
    except Exception as err:
        logging.error(
            'Testing test_train_models failed. Error msg: %s',
            str(err))
